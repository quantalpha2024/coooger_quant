...
for date in unique_dates:
    try:
        # 计算回溯时间窗口
        start_date = date - dt.timedelta(days=31)
        ...
        # 选择表现最好和最差的N个用户
        top_users = user_cumulative_pnl['uid'].iloc[-N:].tolist()
        bottom_users = user_cumulative_pnl['uid'].iloc[:N].tolist()

        # 获取选定用户在当日的交易数据
        top_day_data = df[(df['日期'] == date) & (df['uid'].isin(top_users))]
        bottom_day_data = df[(df['日期'] == date) & (df['uid'].isin(bottom_users))]
